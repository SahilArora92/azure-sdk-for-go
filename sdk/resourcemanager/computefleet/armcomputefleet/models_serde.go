...
// MarshalJSON implements the json.Marshaller interface for type BasePriorityProfile.
func (b BasePriorityProfile) MarshalJSON() ([]byte, error) {
	objectMap := make(map[string]any)
	populate(objectMap, "capacity", b.Capacity)
	populate(objectMap, "minCapacity", b.MinCapacity)
	return json.Marshal(objectMap)
}

// UnmarshalJSON implements the json.Unmarshaller interface for type BasePriorityProfile.
func (b *BasePriorityProfile) UnmarshalJSON(data []byte) error {
	var rawMsg map[string]json.RawMessage
	if err := json.Unmarshal(data, &rawMsg); err != nil {
	...
	for key, val := range rawMsg {
		...
		case "capacity":
			...
			delete(rawMsg, key)
		case "minCapacity":
				err = unpopulate(val, "MinCapacity", &b.MinCapacity)
			delete(rawMsg, key)
		}
		if err != nil {
			return fmt.Errorf("unmarshalling type %T: %v", b, err)
